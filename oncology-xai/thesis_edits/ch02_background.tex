...
This chapter establishes the theoretical foundations underpinning the thesis.
The exposition proceeds from the application domain---digital pathology---through the machine learning and fuzzy logic building blocks that underpin the proposed artefacts, to the design science research methodology that structures the overall investigation.
Medical domain knowledge---histological pattern definitions, oncogenic driver mutations, treatment implications, and clinical datasets---is presented in Chapter~\ref{ch:usecase}.
Each section introduces established concepts from the literature, providing the prerequisite knowledge to follow the implementation and evaluation chapters that follow.
...
At the highest resolution (typically $0.25\,\mu\text{m/pixel}$ at $40\times$), individual cells, their internal structures, and the connective tissue between them are discernible.
At lower magnifications ($10\times$, $5\times$), the overall tissue organisation---whether cells form gland-like tubes, finger-like projections, or dense solid sheets---becomes the dominant visual feature.
...
Computationally, LUAD defines a structured multi-class classification problem at the tile level (six histological growth patterns recognised by the WHO) and a binary prediction problem at the slide level (somatic mutation status per gene).
...
The detailed clinical descriptions of the six histological patterns, their morphological features, grading significance, molecular correlates, and treatment implications are presented in Chapter~\ref{ch:usecase} (Sections~\ref{sec:uc:luad_histology}--\ref{sec:uc:gene_walkthrough}).
...
CTransPath~\cite{wang2022ctranspath} is a hybrid convolutional-transformer model pretrained on approximately 15 million histopathology image tiles from The Cancer Genome Atlas (TCGA)~\cite{tcga2014luad} and the Pathology AI Platform (PAIP) using a semantically relevant contrastive learning objective.
...
\textbf{What the embedding encodes.}
Although the 768 numbers lack individual human-readable labels,
the contrastive pretraining forces them to encode the visual features
that distinguish one tissue region from another.
In practical terms, these dimensions collectively represent
morphological properties that a pathologist would recognise:
nuclear size, shape and chromatin texture;
glandular architecture and lumen formation;
stromal composition and fibrosis;
cell density and spatial arrangement;
and staining intensity gradients.
The embedding is therefore a compact numerical summary of
\emph{how the tissue looks} in each tile, expressed in a form that
downstream models can process.
This distinction is important because, unlike the six explicit pattern
features (Section~\ref{sec:meth:pattern:problem}), the embedding
captures a much richer---but less interpretable---set of
morphological details that are not tied to any predefined category.
...
This formulation combines ArcFace and CosFace margins in a single loss, providing complementary angular and cosine-domain penalties~\cite{deng2019arcface}.
...
The choice of t-norm affects how fuzzy memberships interact during aggregation and inference~\cite{grabisch2009aggregation}.
...
This section formalises the complete family of fuzzy margin variants evaluated in the benchmark (Chapter~\ref{ch:evaluation}), totalling eight loss functions numbered~9--16 in the benchmark enumeration of Table~\ref{tab:loss_variants_bg}.

\subsection{The Motivation: Morphological Ambiguity}
\label{sec:bg:fuzzy_margin:motivation}

In histopathological pattern classification, inter-class boundaries are inherently gradual~\cite{thunnissen2012reproducibility}.
...
Thunnissen et al.\ reported only fair inter-observer agreement ($\kappa = 0.38$) for the predominant pattern among 26~pathologists, with acinar--papillary distinction being particularly difficult~\cite{thunnissen2012reproducibility}.
...
A \emph{fuzzy measure} (also called a capacity)~\cite{sugeno1974theory} on a finite set $N = \{1, 2, \ldots, n\}$ is a set function $g: 2^N \to [0, 1]$ satisfying:
\begin{enumerate}
...
A general fuzzy measure on $n$ elements requires $2^n - 2$ parameters (one per non-empty proper subset)~\cite{grabisch2009aggregation}, which is intractable for even moderate $n$.
The \emph{$k$-additive} approximation~\cite{grabisch1997kapprox} restricts the measure to interactions of order at most $k$.
In this thesis, a 2-additive fuzzy measure is used, parameterised by the Shapley values~\cite{shapley1953value} $\phi_i = g(\{i\})$ and pairwise interaction indices $I_{ij}$:
\begin{equation}
\label{eq:2additive}
...
\textbf{Interpretability advantage:} The Shapley values $\phi_i$ quantify the marginal contribution of each pattern to the fuzzy measure, while the interaction indices $I_{ij}$~\cite{grabisch2009aggregation} reveal whether co-occurrence of two patterns is synergistic (e.g., solid + micropapillary predicting TP53 more than either alone) or redundant.
...
When all Shapley values are near-uniform ($\phi_i \approx 1/n$), the fuzzy measure is approximately additive and the Choquet integral reduces to a simple weighted average~\cite{grabisch2000choquet}---meaning no single pattern is more important than any other in the aggregation.
...
six genes.
For KRAS and RBM10, where FC-MIL is the primary prediction method,
...
even when ABMIL or PI-ABMIL is the selected predictor.
...
% ============================================================================
\section{Interpretability and Explainability in Machine Learning}
\label{sec:bg:xai}
% ============================================================================

The thesis title commits to \emph{interpretable} genotype--phenotype
inference, and the LCHAI system reviewed in
Chapter~\ref{ch:methodology} also produces natural-language
\emph{explanations} via a knowledge-graph--grounded LLM agent.
The two terms are routinely conflated in the clinical-AI
literature~\cite{tjoa2021survey} but denote distinct properties in
the machine-learning community; this section adopts the
distinction explicitly, so that later chapters can be precise
about which property of LCHAI is being claimed and which has
actually been evaluated.

\subsection{Definitions: Interpretability vs.\ Explainability}
\label{sec:bg:xai:definitions}

\textbf{Interpretability} is, following Doshi-Velez and
Kim~\cite{doshivelez2017rigorous}, ``the ability to explain or
to present in understandable terms to a human,'' and is taken to
be a property of the model itself: its internal mechanisms are
either directly inspectable (\emph{transparent} or
\emph{simulatable} in Lipton's
taxonomy~\cite{lipton2018mythos}) or expose semantically
meaningful intermediate quantities that a human can reason about
without an auxiliary explainer.
Rudin~\cite{rudin2019stop} sharpens this view for high-stakes
decisions and argues that, where possible, one should
\emph{design} a model to be intrinsically interpretable rather
than wrap a black box in post-hoc explanations whose faithfulness
is not guaranteed.

\textbf{Explainability}, in contrast, is the production of
human-readable justifications for individual predictions of an
otherwise opaque model, typically through post-hoc tools such as
saliency maps, feature attribution (e.g., SHAP, LIME) or
natural-language rationales~\cite{gilpin2018explaining,arrieta2020xai}.
A model can be explainable without being interpretable (a black
box wrapped in SHAP), and---less commonly---interpretable without
being explainable (a transparent model whose mechanism is exposed
but whose per-prediction rationale is not rendered in natural
language).

\paragraph{Position of LCHAI.}
Under this distinction, the system delivers both, but at different
evidence levels:
\begin{itemize}
    \item \textbf{Interpretability} is a structural property of
    the artefacts.
    The pattern channel exposes a fixed 6-dimensional categorical
    output over expert-validated histological growth patterns
    (Section~\ref{sec:bg:dual_interpretability}); the 2-additive
    Choquet integral of FC-MIL exposes its six singleton Shapley
    values and fifteen pairwise interaction indices as
    \emph{intrinsic} parameters of the trained model
    (Section~\ref{sec:bg:choquet:secondorder}); and the system
    instruments six complementary inspection levels (attention,
    pattern overlay, SHAP, intrinsic Shapley, channel ablation,
    fuzzy linguistic labels) that must be considered jointly to
    reach a conclusion about a slide.
    Interpretability claims of this thesis are therefore
    \emph{empirically evaluated} in
    Chapter~\ref{ch:evaluation} at two distinct levels of evidence:
    (i)~\emph{per-slide}, through the representative-case
    analysis on individual TCGA slides (qualitative inspection of
    the six interpretability outputs against expert morphology); and
    (ii)~\emph{per-gene at the cohort level}, through the
    Conclusive/Inconclusive flag, which is a
    \emph{population-level} (cohort-wide) verdict computed from the
    held-out 5-fold cross-validation AUROC on TCGA-LUAD
    (a gene is Conclusive if and only if AUROC~$\geq 0.70$,
    Hosmer--Lemeshow threshold)
    and is therefore a property of a gene's signal under the
    proposed architecture, not a per-case attribute of any
    individual slide.
    \item \textbf{Explainability} is delivered through the
    knowledge-graph--grounded LLM agent that converts the
    six-level interpretability outputs into natural-language
    explanations constrained by NCIt/MONDO ontology terms and
    fuzzy linguistic labels (Chapter~\ref{ch:methodology};
    Section~\ref{sec:meth:interp:fuzzy_labels}).
    The dual constraint of fuzzy labels (controlling
    \emph{how} the LLM speaks) and the curated knowledge graph
    (controlling \emph{what} it can claim) is intended as an
    anti-hallucination mechanism, but the resulting explanations
    have \emph{not} been formally evaluated in this thesis: no
    reader study, no faithfulness audit against the underlying
    interpretability outputs, and no quantitative explanation-quality
    metric is reported.
    Explainability is therefore claimed only as a
    system capability, not as an evaluated property; a
    formal evaluation is left for future work
    (Section~\ref{sec:disc:future}).
\end{itemize}

The remainder of this section reviews the post-hoc explanation
tools (SHAP, attention) that LCHAI uses as instrumentation, and
then describes the six-level interpretability framework that the
artefacts together expose.
...
\begin{itemize}
    ...
    XGBoost floor baseline (condition~B1) to attribute mutation predictions to
    ...
    (conditions~B2, PI-ABMIL, A) to decompose predictions across the
...
The pipeline targets \emph{interpretable} decision support
(in the sense of Section~\ref{sec:bg:xai:definitions})---a
multi-level inspection layer over pattern-informed mutation
predictions, complemented by an \emph{explainability} layer of
knowledge-graph--grounded natural-language rationales---for the
$\sim$668 publicly available LUAD patients, two orders of magnitude
below proprietary SOTA datasets. \\[4pt]

3. Design evaluation &
The utility, quality, and efficacy of the artefact must be rigorously
demonstrated. &
Evaluation proceeds at five levels:
...
for 6 genes, 6 conditions, 180 models), with a cohort-level
Conclusive/Inconclusive reliability flag derived from the held-out
AUROC (Hosmer--Lemeshow threshold $\geq 0.70$);
(iii)~per-slide six-level interpretability analysis on representative
TCGA cases with trained checkpoints (ABMIL attention maps, pattern
overlays, post-hoc SHAP decomposition, intrinsic Choquet Shapley
values, per-slide feature channel ablation, fuzzy linguistic labels);
(iv)~system-level demonstration of LCHAI~v2.0 showing end-to-end
operation from WSI upload to knowledge graph explanation (RQ6, RQ7);
and (v)~explainability of the LLM--KG agent, claimed only as a
\emph{system capability}, with formal evaluation deferred to future
work (Section~\ref{sec:bg:xai:definitions}). \\[4pt]
...
(v)~\textbf{Analytical contribution}: an empirical taxonomy of
H\&E-mutation signal location across the six TCGA-LUAD targets,
classifying each gene under one of three failure modes when the
ANORAK 6-pattern softmax is the only morphological prior---%
\emph{label-space mismatch}~\cite{lipton2018labelshift} (signal lies
outside the pattern vocabulary; STK11, KEAP1, RBM10), \emph{no
morphological projection} (no consistent H\&E correlate at WSI
resolution; KRAS), and \emph{collapse-to-prior}~\cite{bowman2016posteriorcollapse}
(signal too weak to lift the model off prevalence; low-prevalence
genes); the thesis-specific contribution is the empirical assignment
of genes to mechanisms, not the names themselves;
(vi)~\textbf{LCHAI~v2.0}: integrative clinical prototype with unified
Analysis tab, fuzzy linguistic labels (anti-hallucination), active
learning, and four-level fuzzy coherence (training, representation,
aggregation, interpretation). \\[4pt]
...
PI-ABMIL (ours), one-hot ablation, FC-MIL (ours)); backbone encoders
...
clinician-facing explanations with fuzzy linguistic labels and
PubMed-cited LLM narratives grounded in a biomedical knowledge graph
...
Closing this gap is one of the contributions of the present thesis (Artefacts~2 and~3, PI-ABMIL and FC-MIL); the others---summarised in Table~\ref{tab:dsr_mapping}---are the per-sample fuzzy margin loss (Artefact~1, FuzzyArcLoss~V2), the empirical three-mechanism taxonomy of H\&E--mutation signal location, the ontology-grounded explanation layer (NCIt+MONDO with PMID provenance), and the LCHAI~v2.0 clinical prototype that integrates them.
...
\textbf{STK11} & \textbf{KEAP1} & \textbf{RBM10} \\
\hline
Coudray et al.    & 2018 & Nat.~Med.       &    382 & Inception-v3 e2e & 0.733--0.86\textsuperscript{*} & 0.826 & 0.733 & 0.733\textsuperscript{*} & --- & --- \\
Kather et al.     & 2020 & Nat.~Cancer     & 17{,}355 & ShuffleNet+att. & 0.65--0.72$^\dagger$ & 0.63--0.68$^\dagger$ & 0.60--0.65$^\dagger$ & --- & --- & --- \\
Saldanha et al.   & 2023 & npj Prec.~Onc. &    676 & RetCCL+attMIL    & 0.65$^\dagger$ & 0.67$^\dagger$ & 0.61$^\dagger$ & 0.63$^\dagger$ & 0.58$^\dagger$ & --- \\
Logan et al.      & 2022 & medRxiv         &    747 & CNN+subtypes     & 0.692 & 0.681 & --- & --- & --- & --- \\
Zhao et al.       & 2025 & Lancet Onc.     & 3{,}637 & DeepGEM MIL    & 0.82--0.96 & 0.82--0.96 & 0.82--0.96 & 0.82--0.96 & --- & --- \\
...
\textbf{This thesis} & 2026 & --- & \textbf{687}
  & Pattern-ABMIL  & \textbf{0.717} & \textbf{0.694} & 0.590 & \textbf{0.695} & \textbf{0.610} & 0.640 \\
  & & & & Fuzzy Choquet   & 0.716 & 0.684 & \textbf{0.609} & 0.658 & 0.589 & \textbf{0.661} \\
\hline
\end{tabular}%
}
\begin{flushleft}
...
Saldanha et al.\ STK11/KEAP1 values estimated from supplementary bar charts.
...
Following Doshi-Velez and Kim~\cite{doshivelez2017rigorous},
Lipton~\cite{lipton2018mythos}, and Rudin~\cite{rudin2019stop},
the two terms were formally distinguished
(Section~\ref{sec:bg:xai:definitions}): \emph{interpretability} as
a structural property of the model itself (instrumented in this
thesis through a six-level framework, Section~\ref{sec:bg:dual_interpretability}),
and \emph{explainability} as the post-hoc production of human-readable
rationales (delivered through the LCHAI~v2.0 LLM agent grounded in
the NCIt+MONDO knowledge graph).
Within the interpretability layer, two complementary
Shapley-value--based mechanisms were distinguished
(Table~\ref{tab:shapley_distinction}): SHAP/DeepSHAP as a \emph{post-hoc}
method applied externally to trained models (decomposing predictions
into embedding vs.\ pattern contributions), and the Choquet Shapley
values as \emph{intrinsic} parameters of the learned fuzzy measure
in Artifact~3 (revealing which growth patterns the model globally
associates with each mutation).
...
Against this landscape, the thesis is positioned not as a scale
competition but as a multi-pronged contribution
(Table~\ref{tab:dsr_mapping}, Row~4): \emph{methodological}
(three fuzzy artefacts---FuzzyArcLoss~V2, PI-ABMIL, FC-MIL---%
interpretable and reproducible from public data),
\emph{analytical} (an empirical three-mechanism taxonomy of
H\&E--mutation signal location across the six TCGA-LUAD targets,
established in Chapter~\ref{ch:evaluation}), \emph{explanatory}
(an ontology-grounded knowledge-graph layer with PMID provenance,
RQ6--RQ7), and \emph{integrative} (the LCHAI~v2.0 clinical
prototype, DSR instantiation).

\textbf{Methodology.}
Design Science Research---and its fuzzy extension~\cite{meier2009fuzzy}---provides the research framework that structures the creation and evaluation of the contributions enumerated above, with the seven Hevner guidelines mapped to chapters in Table~\ref{tab:dsr_mapping}.
