...
achieves competitive performance with only 21 parameters on foundation
model embeddings, this would confirm the value of the fuzzy aggregation
mechanism independently of the encoder.
...
In histopathological pattern classification, inter-class boundaries are inherently gradual.
...
For $n = 6$ histological pattern classes, this yields 21 learnable parameters---a tractable number even for the limited training set of approximately 350 slides.
...
(iii)~\textbf{Fuzzy Choquet MIL} (Artifact~3): 21-parameter aggregation
with intrinsic Shapley values and interaction indices;
