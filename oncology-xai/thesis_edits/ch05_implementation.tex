...
The input dimension varies by condition: 512 (B2), 6 (B3), 518 (P and A).
...
Concat dim.         & Total input (proposed condition P)  & 518 \\
...
\begin{enumerate}
    ...
    $c \in \{B1, B2, B3, P, A, FC\}$ (6 conditions).
    ...
    \begin{enumerate}
        ...
        \item For conditions B2, B3, P, A: train an ABMIL model for
        50 epochs.
        \item For condition FC: train the Fuzzy Choquet MIL model for
        50 epochs.
        \item Evaluate on the test fold: compute AUROC and F1
        at threshold 0.5 (AUPRC is additionally computed for the
        XGBoost baseline).
...
\centering
\small
\caption{Hyperparameters for the Fuzzy Choquet MIL (condition~FC).
...
Displays the gene name, $P(\text{mut})$ as a percentage, a confidence label (\emph{Conclusive} if fold-AUROC~$\geq 0.70$, \emph{Inconclusive} otherwise), the prediction method (B2 embeddings, Proposed concat, or Fuzzy Choquet), and the model AUROC.
...
A stacked bar shows the relative SHAP contribution of embedding dimensions (0--511) versus pattern dimensions (512--517).
...
Three vertical bars compare $P(\text{mut})$ from three independently trained models: Combined (518-d), Embedding-only (512-d), and Pattern-only (6-d).
...
\caption[LCHAI~v2.0 SHAP Decomposition --- STK11]{%
    SHAP decomposition for STK11 (Proposed concat method) on
...
\caption[LCHAI~v2.0 Admin --- System Parameters]{%
    ...
    thresholds, prediction method selection (B2/P/FC), and inference parameters
...
generates a per-gene narrative comparing ablation results,
SHAP decomposition, and Choquet Shapley values.
The prompt includes ablation data (whether patterns help or
harm this gene), the knowledge graph's pattern--gene associations,
and the slide's morphological profile.
For the Choquet explanation, the prompt additionally instructs the
LLM to assess whether singleton Shapley values deviate meaningfully
from uniform ($1/6$) and to focus on interaction indices.

An example graph-level explanation for TCGA-55-7815:
\begin{quote}
\emph{``Histological analysis of this lung adenocarcinoma slide
(1,708 tiles analysed) reveals a micropapillary-predominant pattern
(79.4\% of tiles).  Secondary patterns include acinar (17.1\%),
cribriform (2.5\%), and solid (0.6\%).
Mutation predictions (reliable): TP53 (61.3\%)---this gene has a
reliable model (AUROC~$\geq 0.70$) and the prediction is considered
conclusive.  TP53 mutations are commonly associated with solid and
micropapillary subtypes~\cite{leighl2002tp53morphology}.
Likely poorly differentiated (micropapillary-predominant,
SSR~0.97).''}
\end{quote}

An example gene-level explanation for KRAS on the same case:
\begin{quote}
\emph{``The combined model's superior performance (65.8\%) compared to
the embeddings-only model (60\%) suggests potential synergy between
slide-level morphologic features and histological growth patterns,
enhancing the model's overall candidate signal for KRAS mutation.
The significant drop in performance of the patterns-only model
(16.5\%) indicates that patterns alone are insufficient.
High predictive uncertainty (AUROC~$<0.70$) means molecular testing
is recommended.''}
\end{quote}
