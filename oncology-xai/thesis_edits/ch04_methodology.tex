...
Section~\ref{sec:meth:mutation_prediction} presents the mutation prediction framework, detailing the five experimental conditions: the XGBoost baseline (B1), ABMIL on embeddings (B2), ABMIL on patterns (B3), PI-ABMIL (PI, Artefact~2, ours), the crisp ablation (A), and FC-MIL (FC, Artefact~3, ours).
...
This distinction is central to the SHAP decomposition
(Section~\ref{sec:meth:shap}), where the relative contribution of the
...
PI  & PI-ABMIL (concat soft, ours) & $\mathbf{h}_i = [\mathbf{x}_i \,\|\, \mathbf{p}_i]$ & 518 \\
A  & Ablation (one-hot) & $\mathbf{h}_i = [\mathbf{x}_i \,\|\, \mathbf{o}_i]$ & 518 \\
FC & FC-MIL (hybrid, ours) & $\mathbf{h}_i = (\mathbf{x}_i, \mathbf{p}_i)$ (dual pathway) & 512+6 \\
\hline
\end{tabular}
\end{table}

\textbf{The key comparison} is between condition PI (PI-ABMIL) and condition B2 (standard ABMIL).
If PI outperforms B2, the fuzzy pattern probabilities contribute information beyond what the CTransPath embeddings encode implicitly.
The comparison between PI and A tests whether the continuous (fuzzy) representation $\mathbf{p}_i$ is more informative than the discrete (crisp) representation $\mathbf{o}_i$.

\subsection{Attention-Based MIL Architecture}
\label{sec:meth:mutation:abmil}

For conditions B2, B3, PI, and A, the ABMIL architecture consists of three modules: a tile-level transformation, an attention mechanism, and a slide-level classifier.
...
performance difference between conditions~PI and~FC is attributable to
...
For ABMIL-based conditions (B2, B3, PI, A), the attention weights $\{a_i\}$ from Equation~\ref{eq:gated_attn} are overlaid onto the WSI as a heatmap.
...
For conditions that incorporate pattern information (B3, PI, A, FC), each tile's predicted pattern class $\hat{y}_i$ is overlaid onto the WSI as a categorical colour map.
...
In condition PI (PI-ABMIL), the SHAP decomposition can distinguish contributions from the CTransPath embedding dimensions (positions 1--512) versus the pattern probability dimensions (positions 513--518), quantifying the relative importance of visual features versus pattern knowledge for each gene.
...
\begin{enumerate}
    ...
    (condition~PI), operating on the full 518-d representation
...
All six experimental conditions (B1: XGBoost on pattern features; B2: ABMIL on embeddings; B3: ABMIL on patterns; PI: PI-ABMIL, ours; A: crisp ablation; FC: FC-MIL, ours) share the same fold splits.
Conditions B2, B3, PI and A evaluate Artefact~2 under different input representations, while condition FC evaluates Artefact~3.
...
\begin{enumerate}
    \item \textbf{Does pattern information help at all?} $\text{PI} > B2$ and $B3 > \text{random}$.
    \item \textbf{Are embeddings necessary alongside patterns?} $\text{PI} > B3$ and $B2 > B3$.
    \item \textbf{Are soft (fuzzy) probabilities better than hard (crisp) labels?} $\text{PI} > A$.
    \item \textbf{Is MIL necessary beyond slide-level aggregation?} $B2 > B1$.
    \item \textbf{Does the Choquet branch add value over standard attention?} $\text{FC}$ vs $\text{PI}$ per gene. Because FC shares the embedding pathway with ABMIL, this isolates the contribution of the Choquet aggregation. Shapley interaction analysis provides a qualitative plausibility check.
    \item \textbf{Is the hybrid Choquet model competitive with concatenation?} $\text{FC}$ vs $\text{PI}$, with the advantage that FC provides intrinsic interpretability (Shapley values and interaction indices) absent from~PI.
\end{enumerate}
