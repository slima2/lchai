...
This chapter presents the methodological design of the three computational artefacts introduced in Section~\ref{sec:uc:artefact_vision}.
...
Section~\ref{sec:meth:mutation_prediction} presents the mutation prediction framework, detailing the five experimental conditions: the XGBoost baseline (B1), ABMIL on embeddings (B2), ABMIL on patterns (B3), the proposed pattern-informed ABMIL (P), the crisp ablation (A), and the Fuzzy Choquet MIL (FC, Artefact~3).
...
Given a WSI in Aperio SVS format, tissue regions are segmented from the glass background using Otsu thresholding on the saturation channel of the HSV colour space.
...
A learned linear projection head, fine-tuned during the pattern classification training on the Zenodo dataset, compresses the representation to 512 dimensions:
\begin{equation}
\label{eq:projection}
...
The projection from 768 to 512 dimensions was optimised jointly with the FuzzyArcLoss~V2 objective during fine-tuning on the six LUAD histological patterns.
The resulting embedding space is therefore \emph{pattern-oriented}: its geometry reflects the angular separation of the six histological classes rather than the generic histopathological features of the pretrained CTransPath model.
While the literature uses embedding dimensions ranging from 512 (CTransPath) to 2048 (RetCCL~\cite{wang2023retccl}), lower dimensionality is advantageous under data scarcity because it reduces the risk of overfitting in downstream models operating on $\sim$687 slides.
...
Ablation experiments (Section~\ref{sec:uc:pipeline_fixes}) demonstrated that this modification contributes an estimated 15--20 percentage points to classification performance.
...
to the class-imbalanced Zenodo--ANORAK dataset where the minority
...
\midrule
ArcFace & Fixed additive & None & --- \\
CosFace & Fixed cosine & None & --- \\
SphereFace & Fixed multiplicative & None & --- \\
AdaptiveFace & Class-adaptive & Per class & $\bar{c}_k$ (class difficulty) \\
CurricularFace & Curriculum-adaptive & Per sample & $t$ (EMA of $\cos\theta_y$) + sample $\cos\theta_j$ \\
...
AdaptiveFace adjusts margins per class but treats all samples within a class uniformly.
CurricularFace is also per-sample, but it modulates \emph{negative-class} logits based on an adaptive threshold $t$ (an exponential moving average of target cosine) that evolves during training; the target-class margin itself remains fixed at $m$.
...
The three key hyperparameters of FuzzyArcLoss~V2---scale $s$, margin $m$, and threshold $\tau$---were optimised using Optuna~\cite{akiba2019optuna}, a Bayesian hyperparameter optimisation framework based on the Tree-structured Parzen Estimator (TPE) algorithm.
...
\begin{itemize}
    \item \textbf{Optimiser:} AdamW with weight decay $10^{-4}$.
    \item \textbf{Learning rate schedule:} Linear warmup for 5 epochs followed by cosine annealing to $10^{-6}$.
    ...
    \item \textbf{Augmentation:} Random horizontal/vertical flip, random rotation ($\pm 90°$), colour jitter (brightness, contrast, saturation, hue), Gaussian blur. No MixUp or CutMix.
...
P  & Proposed (concat soft) & $\mathbf{h}_i = [\mathbf{x}_i \,\|\, \mathbf{p}_i]$ & 518 \\
A  & Ablation (one-hot) & $\mathbf{h}_i = [\mathbf{x}_i \,\|\, \mathbf{o}_i]$ & 518 \\
FC & Fuzzy Choquet (hybrid) & $\mathbf{h}_i = (\mathbf{x}_i, \mathbf{p}_i)$ (dual pathway) & 512+6 \\
\hline
\end{tabular}
\end{table}

\textbf{The key comparison} is between condition P (proposed) and condition B2 (standard ABMIL).
If P outperforms B2, the fuzzy pattern probabilities contribute information beyond what the CTransPath embeddings encode implicitly.
The comparison between P and A tests whether the continuous (fuzzy) representation $\mathbf{p}_i$ is more informative than the discrete (crisp) representation $\mathbf{o}_i$.

\subsection{Attention-Based MIL Architecture}
\label{sec:meth:mutation:abmil}

For conditions B2, B3, P, and A, the ABMIL architecture consists of three modules: a tile-level transformation, an attention mechanism, and a slide-level classifier.
...
Dropout with $p = 0.25$ is applied after each ReLU activation.

\subsubsection{Gated Attention Mechanism}

The attention weight for tile $i$ is computed using the gated attention formulation~\cite{lu2021data}:
\begin{equation}
\label{eq:gated_attn}
...
XGBoost is chosen for its robustness to small sample sizes, built-in regularisation, and compatibility with SHAP for feature attribution.
...
the heterogeneous TCGA-LUAD cohort.
...
The Choquet integral provides exactly this capability.
...
discrete Choquet integral with a learned 2-additive fuzzy measure $g$.
...
The Shapley values are constrained to be non-negative and sum to~1 via a
...
\centering
\small
\caption{Parameter count comparison: ABMIL (condition~P) vs Fuzzy
Choquet MIL (condition~FC).}
\label{tab:param_count}
\begin{tabular}{l r r}
\hline
\textbf{Component} & \textbf{ABMIL (P)} & \textbf{Choquet (FC)} \\
\hline
...
performance difference between conditions~P and~FC is attributable to
...
Unlike post-hoc explanation methods (SHAP, attention maps), the Choquet integral provides built-in interpretability through its learned parameters:
\begin{itemize}
...
For ABMIL-based conditions (B2, B3, P, A), the attention weights $\{a_i\}$ from Equation~\ref{eq:gated_attn} are overlaid onto the WSI as a heatmap.
...
For conditions that incorporate pattern information (B3, P, A, FC), each tile's predicted pattern class $\hat{y}_i$ is overlaid onto the WSI as a categorical colour map.
...
For ABMIL conditions, DeepSHAP attributes the prediction to individual dimensions of the tile representation $\mathbf{h}_i$.
In the proposed condition P, the SHAP decomposition can distinguish contributions from the CTransPath embedding dimensions (positions 1--512) versus the pattern probability dimensions (positions 513--518), quantifying the relative importance of visual features versus pattern knowledge for each gene.

\subsection{Level 4: Choquet Shapley Values (Artefact 3 Only)}
\label{sec:meth:interp:choquet_shapley}

For the Fuzzy Choquet condition (FC), the learned Shapley values and interaction indices provide a fourth level of interpretability that is \emph{intrinsic} to the model rather than post-hoc.
...
\begin{enumerate}
    \item \textbf{Combined} ($f_{\text{P}}$): the proposed concat model
    (condition~P), operating on the full 518-d representation
...
P_{\text{comb}} = \sigma(f_{\text{P}}(\{\mathbf{h}_i\})), \quad
...
All six experimental conditions (B1: XGBoost on pattern features; B2: ABMIL on embeddings; B3: ABMIL on patterns; P: Pattern-Informed ABMIL; A: crisp ablation; FC: Fuzzy Choquet MIL) share the same fold splits.
Conditions B2, B3, P and A evaluate Artefact~2 under different input representations, while condition FC evaluates Artefact~3.
...
literature, reported as mean $\pm$ standard deviation across folds.

\textbf{Secondary metrics:} AUPRC (Area Under the Precision-Recall
...
threshold optimisation (e.g., Youden's $J$) would be needed for
clinical deployment.
AUPRC is reported for the XGBoost baseline (B1).
...
Following standard conventions: $|d| < 0.2$ = negligible, $0.2 \leq |d| < 0.5$ = small, $0.5 \leq |d| < 0.8$ = medium, $|d| \geq 0.8$ = large.
...
\begin{enumerate}
    \item \textbf{Does pattern information help at all?} $P > B2$ and $B3 > \text{random}$.
    \item \textbf{Are embeddings necessary alongside patterns?} $P > B3$ and $B2 > B3$.
    \item \textbf{Are soft (fuzzy) probabilities better than hard (crisp) labels?} $P > A$.
    \item \textbf{Is MIL necessary beyond slide-level aggregation?} $B2 > B1$.
    \item \textbf{Does the Choquet branch add value over standard attention?} $FC$ vs $P$ per gene. Because FC shares the embedding pathway with ABMIL, this isolates the contribution of the Choquet aggregation. Shapley interaction analysis provides a qualitative plausibility check.
    \item \textbf{Is the hybrid Choquet model competitive with concatenation?} $FC$ vs $P$, with the advantage that FC provides intrinsic interpretability (Shapley values and interaction indices) absent from~P.
\end{enumerate}
