...
# V2 Optuna (separate search, Feb 2026):
# Trial 30 finished with value: 0.9306 and parameters: {'S_SCALE': 46.101434376822986, 'M_MARGIN': 0.5178476475091369, 'TAU': 0.44787389912216496, 'BACKBONE_LR': 7.478950876910481e-05, 'HEAD_LR_MULT': 8.282463948665386, 'FREEZE_LAYERS': 2, 'WARMUP_EPOCHS': 8, 'USE_FOCAL_CE': False, 'FOCAL_GAMMA': 2.115}. Best is trial 30 with value: 0.9306.
...
try:
    ...
    HAS_KORNIA = False
    print("[WARNING] kornia not installed. pip install kornia — falling back to CPU augmentations")
...
class AblationConfig:
    ...
    EPOCHS: int = 120
    BATCH_SIZE: int = 8            # per-GPU; effective = 8 * num_gpus
    ...
    USE_MASK_AS_CHANNEL: bool = True  # 4-channel input (RGB+mask) — critical for performance
    
    # Data splits (match Optuna notebook: 80/20 train/val, no separate test)
    VAL_SIZE: float = 0.20
    TEST_SIZE: float = 0.0  # no separate test set — val IS test (matches notebook)
    SEED: int = 42
    
    # Distributed
    NUM_WORKERS: int = 0
    MIXED_PRECISION: str = "no"  # notebook trains in fp32 — fp16 hurts V3 SubCenters
    
    # Augmentation — NO MixUp/CutMix (notebook doesn't use them, they hurt V3 SubCenters)
...
# ==============================================================================
# FOCAL CE (from Optuna notebook — achieves 91% avg_pr)
# ==============================================================================

class FocalCE(nn.Module):
    """
    Focal Cross-Entropy with per-class alpha weighting.
    This is the EXACT loss used in the notebook that achieved 91% avg_pr.
    alpha = [1/count_i for each class], normalized to sum=1
    """
    ...
    def forward(self, logits, target):
        ...
        pt = logp.exp()[torch.arange(logits.size(0), device=logits.device), target]
...
class FuzzyArcLossV3SubCenters(nn.Module):
    ...
    Uses FocalCE with class-balanced alpha (matching Optuna notebook that achieved 91%)."""
    def __init__(self, in_features: int, out_features: int,
                 s: float = 30.0, m: float = 0.50, tau: float = 0.5,
                 num_subcenters: int = 3,
                 label_smoothing: float = 0.1, ce_weight: torch.Tensor = None,
        ...
        # Use FocalCE with class-balanced alpha (same as notebook)
        if class_counts is not None:
            ...
            alpha = None
        self.ce = FocalCE(alpha=alpha, gamma=focal_gamma)
        self.s = s
        self.weight = self.head.weight
...
class CTransPathBackbone(nn.Module):
    """CTransPath backbone with ConvStem. Supports 3 or 4 channel input."""
    
    def __init__(self, checkpoint_path: str = None, freeze_layers: int = 0, in_channels: int = 3):
        super().__init__()
        ...
        embed_dim = 96
        # First load with 3 channels to match checkpoint weights
        ...
        # AFTER loading checkpoint, modify first conv for 4-channel input
        if in_channels != 3:
            old_conv = self.model.patch_embed.proj[0]  # First Conv2d in ConvStem
            new_conv = nn.Conv2d(
                in_channels, old_conv.out_channels,
                kernel_size=old_conv.kernel_size, stride=old_conv.stride,
                padding=old_conv.padding, bias=old_conv.bias is not None
            )
            # Copy RGB weights and init extra channel(s) from mean of RGB
            with torch.no_grad():
            ...
            print(f"  ✓ Modified ConvStem for {in_channels}-ch input")
...
def build_model_with_loss(loss_type: str, num_classes: int, embed_dim: int,
    ...
    # Backbone
    backbone = CTransPathBackbone(checkpoint_path, freeze_layers, in_channels=in_channels)
...
def normalize_key(stem: str) -> str:
    """Normalize filename stem for matching (from notebook)."""
    s = stem.lower()
...
def list_images(d: str) -> List[Path]:
    return sorted([p for p in Path(d).rglob("*") if p.is_file() and p.suffix.lower() in IMG_EXTS])
...
def pair_images_with_masks(image_dir: str, mask_dir: str) -> pd.DataFrame:
    """Pair images with masks using robust matching (from notebook)."""
    ...
    for ip in imgs:
        ...
        if mp:
            rows.append({"image_path": str(ip), "mask_path": mp, "base": stem, "base_norm": key_norm, "stem": stem})
        else:
...
class StainAugmentation:
    """Stain augmentation for H&E histopathology images (CPU fallback)."""
...
# ==============================================================================
# MASK LOADING (from Optuna notebook — supports .mat with h5py fallback)
# ==============================================================================

MAT_PRIOR = ["mask","Mask","BW","bw","label","Label","roi","ROI","seg","Seg","segmentation"]

def _load_mat_any(path: str) -> np.ndarray:
    """Load any 2D array from .mat file (scipy or h5py)."""
    ...
    if h5py is not None:
        try:
            with h5py.File(path, "r") as f:
                ...
                f.visititems(visit)
                if best is not None: return np.array(best[()])
        except Exception: pass
    raise RuntimeError(f"No readable 2D/3D dataset in .mat: {path}")

def load_mask_from_mat(path: str, thresh: float = 0.5) -> Image.Image:
    """Load mask from .mat file, return as PIL Image (mode L)."""
    ext = os.path.splitext(path)[1].lower()
    if ext == '.mat':
        arr = np.array(_load_mat_any(path))
        while arr.ndim > 2 and arr.shape[-1] == 1: arr = arr[..., 0]
        if arr.ndim > 2:
...
# ==============================================================================
# HISTMASK DATASET (4-channel: RGB + mask — matches Optuna notebook)
# ==============================================================================

class HistMaskDataset(Dataset):
    """Histopathology dataset with mask as 4th channel + joint augmentation."""
    def __init__(self, rows, label_col, label2id, img_size=224, aug=True,
                 use_mask_as_channel=True, use_roi_crop=False, roi_padding=16, mask_thresh=0.5):
        ...
        self.cj = T.ColorJitter(0.2, 0.2, 0.2, 0.05)

    def __len__(self): return len(self.rows)

    def _roi_crop(self, img, msk):
        m = np.array(msk) > 0
    ...
    def _joint(self, img, msk):
        if self.aug:
            i, j, h, w = T.RandomResizedCrop.get_params(img, scale=(0.6,1.0), ratio=(0.9,1.1))
    ...
    def __getitem__(self, idx):
        ...
        if self.use_roi_crop: img, msk = self._roi_crop(img, msk)
        img, msk = self._joint(img, msk)
        if self.aug: img = self.cj(img)
...
# ==============================================================================
# GPU-CACHED MASK DATASET (pre-loads 4-ch tensors, GPU augmentation)
# ==============================================================================

class GPUCachedMaskDataset(Dataset):
    """
    Pre-loads ALL images + masks into CPU memory as 4-channel tensors.
    No disk I/O during training. Augmentations happen on GPU after collation.
    """
    def __init__(self, df: pd.DataFrame, label_col: str, label2id: Dict,
                 img_size: int = 224, use_mask_as_channel: bool = True,
                 use_roi_crop: bool = True, roi_padding: int = 24,
                 mask_thresh: float = 0.5):
        self.labels = [label2id[str(row[label_col])] for _, row in df.iterrows()]
        self.use_mask_as_channel = use_mask_as_channel

        self.images = []
        print(f"    Pre-caching {len(df)} images+masks to memory...", end=" ", flush=True)
        for _, row in df.iterrows():
            img = Image.open(row['image_path']).convert('RGB')
            msk = load_mask_from_mat(row['mask_path'])

            # ROI crop (one-time cost during pre-load)
            if use_roi_crop:
                m = np.array(msk) > 0
                if m.any():
                    ys, xs = np.where(m)
                    h, w = m.shape
                    y0 = max(0, ys.min() - roi_padding)
                    y1 = min(h, ys.max()+1 + roi_padding)
                    x0 = max(0, xs.min() - roi_padding)
                    x1 = min(w, xs.max()+1 + roi_padding)
                    img = img.crop((x0,y0,x1,y1))
                    msk = msk.crop((x0,y0,x1,y1))

            # Resize
            img = img.resize((img_size, img_size), resample=Image.BILINEAR)
            msk = msk.resize((img_size, img_size), resample=Image.NEAREST)

            # To tensor + normalize
            img_t = TF.to_tensor(img)
            img_t = TF.normalize(img_t, mean=[0.485,0.456,0.406], std=[0.229,0.224,0.225])
            msk_t = (TF.to_tensor(msk) > mask_thresh).float()

            if use_mask_as_channel:
                x = torch.cat([img_t, msk_t], dim=0)  # (4, H, W)
            else:
                x = img_t  # (3, H, W)
...
class GPUAugmentation(nn.Module):
    """GPU-accelerated augmentation pipeline using Kornia. Handles 3 or 4 channel input."""
    def __init__(self, img_size: int = 224, training: bool = True):
        super().__init__()
        if training:
            # Geometric augmentations (applied to ALL channels including mask)
            # Matches notebook: RandomResizedCrop + HFlip + VFlip + Rotate(-15,15)
            self.geo_aug = nn.Sequential(
                ...
                K_aug.RandomResizedCrop(size=(img_size, img_size), scale=(0.6, 1.0), ratio=(0.9, 1.1), p=0.5),
            )
            # Color augmentations (applied to RGB channels ONLY)
            # Matches notebook: ColorJitter(0.2,0.2,0.2,0.05) + GaussianBlur(0.3)
            # NOTE: Sharpness and Posterize from notebook not available in Kornia batch ops
            self.color_aug = nn.Sequential(
                K_aug.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.05, p=0.7),
                K_aug.RandomGaussianBlur(kernel_size=(3, 3), sigma=(0.1, 1.0), p=0.3),
            )
            self.stain_aug = False  # notebook doesn't use separate stain aug
        else:
            self.geo_aug = None
            self.color_aug = None
            self.stain_aug = False

    @torch.no_grad()
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """x: (B, C, H, W) where C is 3 or 4. If 4, channel 3 is the mask."""
        if self.geo_aug is None:
            return x

        has_mask = (x.size(1) == 4)

        # 1. Geometric augmentation on ALL channels (keeps mask aligned)
        x = self.geo_aug(x)

        # 2. Stain augmentation on RGB only
        if self.stain_aug:
            B = x.size(0)
            mask = torch.rand(B, device=x.device) < 0.5
            if mask.any():
                scales = torch.ones(B, 3, 1, 1, device=x.device)
                scales[mask, 0, 0, 0] = 1.0 + (torch.rand(mask.sum(), device=x.device) * 0.2 - 0.1)
                scales[mask, 2, 0, 0] = 1.0 + (torch.rand(mask.sum(), device=x.device) * 0.2 - 0.1)
                x[:, :3] = x[:, :3] * scales

        # 3. Color augmentation on RGB only
        if has_mask:
            rgb = x[:, :3]
            msk = x[:, 3:4]
            rgb = self.color_aug(rgb)
            x = torch.cat([rgb, msk], dim=1)
        else:
            x = self.color_aug(x)
...
class GPUCachedDataset(Dataset):
    """
    Pre-loads ALL images into CPU memory as tensors (resized + ROI-cropped).
    No disk I/O during training — only tensor indexing.
    Augmentations happen on GPU after batch collation.
    """
    def __init__(self, df: pd.DataFrame, label_col: str, label2id: Dict,
                 img_size: int = 224, use_roi_crop: bool = True,
                 roi_padding: int = 24, mask_thresh: float = 0.5):
        self.labels = [label2id[str(row[label_col])] for _, row in df.iterrows()]

        # Pre-load all images as tensors (resize + normalize only)
        to_tensor = T.Compose([
        ...
        self.images = []
        print(f"    Pre-caching {len(df)} images to memory...", end=" ", flush=True)
        for _, row in df.iterrows():
            img = Image.open(row['image_path']).convert('RGB')

            # ROI crop (still on CPU during pre-load — one-time cost)
            if use_roi_crop and 'mask_path' in row:
                mask = load_mask(row['mask_path'], mask_thresh)
                if mask is not None:
                    ...
                    if len(xs) > 0:
                        ...
                        x2 = min(w, xs.max() + roi_padding)
                        y2 = min(h, ys.max() + roi_padding)
...
# Keep original HistDataset as fallback when kornia not available
class HistDataset(Dataset):
    """Histopathology dataset with CPU augmentation (fallback)."""
    ...
    def __getitem__(self, idx):
        ...
        if self.use_roi_crop and 'mask_path' in row:
            ...
            if mask is not None:
                ...
                if len(xs) > 0:
                    h, w = mask.shape
                    x1 = max(0, xs.min() - self.roi_padding)
                    y1 = max(0, ys.min() - self.roi_padding)
                    x2 = min(w, xs.max() + self.roi_padding)
                    y2 = min(h, ys.max() + self.roi_padding)
                    img = img.crop((x1, y1, x2, y2))
...
# ==============================================================================
# MARGIN-FREE LOGITS (from Optuna notebook — critical for accurate predictions)
# ==============================================================================

def _logits_nomargin(feats, head):
    """Compute cosine logits WITHOUT arc-margin for prediction.
    Arc-margin logits distort argmax — only valid for loss computation."""
    feats_norm = F.normalize(feats, dim=1)
    weight_norm = F.normalize(head.weight, dim=2)  # (C, K, D) for sub-centers
    cosine_all = torch.einsum('bd,ckd->bck', feats_norm, weight_norm)
    cosine, _ = cosine_all.max(dim=2)
    return cosine * head.s


def _logits_nomargin_any(feats, loss_fn):
    """Get margin-free logits for any loss function type."""
...
def run_single_experiment(loss_type: str, train_loader, val_loader, test_loader,
                          ...
                          gpu_train_aug=None, gpu_val_aug=None,
                          **loss_kwargs) -> Dict:
    ...
    model, loss_fn = build_model_with_loss(
        ...
        class_counts, device, ce_weight=ce_weight,
    ...
    def lr_lambda(epoch):
        if epoch < config.WARMUP_EPOCHS:
            return (epoch + 1) / config.WARMUP_EPOCHS  # match notebook: starts from LR/warmup, not 0
        else:
    ...
    # Mixed precision scaler — disabled to match notebook (fp32 training)
    use_amp = config.MIXED_PRECISION == "fp16"
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            # GPU augmentation (Kornia — all on GPU, no CPU overhead)
            ...
            if scaler is not None:
                ...
                # No grad clipping — notebook doesn't use it
            ...
            # Use margin-free logits for train predictions (margin distorts argmax)
            if not use_mix:
                with torch.no_grad():
                    clean_logits = _logits_nomargin_any(features.detach(), loss_fn)
        ...
        with torch.no_grad():
            for images, labels in val_loader:
                ...
                # Compute val loss
                loss_val, _ = loss_fn(features, labels)
                val_loss_total += loss_val.item() * images.size(0)
                
                # Margin-free cosine logits for predictions (same as notebook)
                clean_logits = _logits_nomargin_any(features, loss_fn)
                if clean_logits is not None:
                    preds = clean_logits.argmax(dim=1)
                else:
                    _, logits = loss_fn(features, labels)
                    preds = logits.argmax(dim=1)
        ...
        val_prec = precision_score(val_labels_arr, val_preds, average='macro', zero_division=0)
        val_rec = recall_score(val_labels_arr, val_preds, average='macro', zero_division=0)
        val_avg_pr = (val_prec + val_rec) / 2.0
        
        # Best model selection on avg_pr (matching notebook)
        if val_avg_pr > best_val_f1:  # var name kept for compat, but tracks avg_pr
            best_val_f1 = val_avg_pr
            best_epoch = epoch
            if isinstance(model, nn.DataParallel):
        ...
        if verbose and (epoch + 1) % config.PRINT_FREQ == 0:
            print(f"  Ep {epoch+1:3d}/{epochs} | Loss {train_loss:.4f} | Val-F1 {val_f1:.4f} P {val_prec:.3f} R {val_rec:.3f} avg_pr {val_avg_pr:.4f} | Best {best_val_f1:.4f}@{best_epoch+1}")
            sys.stdout.flush()
    ...
    with torch.no_grad():
        for images, labels in test_loader:
            ...
            # Use margin-free logits (same as val and notebook)
            clean_logits = _logits_nomargin_any(features, loss_fn)
            if clean_logits is not None:
                preds = clean_logits.argmax(dim=1)
            else:
    ...
    return {
        ...
        'best_state': best_state,  # model + loss_fn state_dicts for checkpoint saving
    }
...
def run_ablation_study():
    ...
    # Load labels from Excel (matching notebook's read_labels_from_xls approach)
    xls = pd.read_excel(config.XLS_PATH)
    
    # Auto-detect columns (same logic as notebook)
    ...
    # Build label map with normalized key fallback (same as notebook)
    label_map = {}
    ...
    # Map labels to df using base + base_norm fallback
    ...
    print(f"  After pattern filter: {len(df)} rows, classes={sorted(list(inc))}")
    ...
    # Split data (80/20 train/val — matching Optuna notebook)
    if config.TEST_SIZE > 0:
        ...
        train_df, val_df = train_test_split(
            df, test_size=config.VAL_SIZE,
            stratify=df[label_col], random_state=config.SEED
        )
        test_df = val_df  # val IS test (same as notebook)
    
    if config.TEST_SIZE > 0:
        print(f"  Train: {len(train_df)}, Val: {len(val_df)}, Test: {len(test_df)}")
    else:
        print(f"  Train: {len(train_df)}, Val/Test: {len(val_df)} (80/20 split, no separate test)")
    ...
    # Create datasets — GPU-cached if kornia available (MUCH faster)
    in_channels = 4 if config.USE_MASK_AS_CHANNEL else 3
    print(f"  Input channels: {in_channels} ({'RGB+mask' if in_channels==4 else 'RGB only'})")
    
    if config.TEST_SIZE > 0:
        test_ds_needed = True
    else:
        test_ds_needed = False  # val IS test — reuse val_ds
    
    if HAS_KORNIA:
        print("  [GPU MODE] Pre-caching images+masks to memory + GPU augmentations via Kornia")
        ...
        if test_ds_needed:
            ...
            test_ds = val_ds  # reuse — no extra caching
        # GPU augmentation modules
        ...
        print("  [CPU MODE] Using HistMaskDataset (install kornia for GPU speedup)")
        ...
        if test_ds_needed:
            ...
            test_ds = val_ds  # reuse
        gpu_train_aug = None
        gpu_val_aug = None
    
    # Create dataloaders
    # With GPU-cached dataset, NUM_WORKERS=0 is fine (no disk I/O to parallelize)
    num_workers = 0 if HAS_KORNIA else config.NUM_WORKERS
    batch_size = config.BATCH_SIZE * max(1, torch.cuda.device_count())
    
    # Notebook uses shuffle=True, NOT WeightedRandomSampler
    USE_WEIGHTED_SAMPLER = False  # override config — match notebook
    ...
    if USE_WEIGHTED_SAMPLER:
        ...
        train_loader = DataLoader(train_ds, batch_size=batch_size,
                                  ...
                                  pin_memory=not HAS_KORNIA, drop_last=True)
    else:
        train_loader = DataLoader(train_ds, batch_size=batch_size,
    ...
    loss_functions = [
        # BEST PERFORMERS — Optuna-optimized
        ('fuzzyarcloss_v2', {'s': 46.101434376822986, 'm': 0.5178476475091369, 'tau': 0.44787389912216496}),  # Optuna Trial 30: avgPR=0.9306
        ('fuzzyarcloss_v3_subcenters', {'s': 39.001786002638916, 'm': 0.5573158242327586, 'tau': 0.4542757061233031, 'num_subcenters': 4}),  # Optuna Trial 9: avgPR=0.9127
        ...
        # V2 variants (V2 Optuna-tuned is already at top of list)
    ...
    for idx, (loss_type, kwargs) in enumerate(loss_functions):
        ...
        try:
            result = run_single_experiment(
                ...
                gpu_train_aug=gpu_train_aug, gpu_val_aug=gpu_val_aug,
                label_smoothing=0.1,
                use_focal=True,
                focal_gamma=2.0,
                **kwargs
            ...
            # Save .pth checkpoint for every trained model (for inference later)
            if result.get('best_state') is not None:
                os.makedirs(config.OUT_DIR, exist_ok=True)  # Ensure directory exists
                ckpt_path = os.path.join(config.OUT_DIR, f"best_{loss_type}.pth")
                torch.save({
                    'model': result['best_state']['model'],
                    'loss_fn': result['best_state']['loss_fn'],
                    'config': {
                        'EMBED_DIM': config.EMBED_DIM,
                        'IMG_SIZE': config.IMG_SIZE,
                        'USE_MASK_AS_CHANNEL': config.USE_MASK_AS_CHANNEL,
                        'S_SCALE': kwargs.get('s', 30.0),
                        'M_MARGIN': kwargs.get('m', 0.5),
                ...
                # Free memory — don't keep state_dicts in the results list
                del result['best_state']
