...
Version 11 - January 2026
Enhanced with FuzzyArcLoss V2, V2.5, V3 variants based on ablation study findings.
...
class AblationConfig:
    ...
    OUT_DIR: str = "/home/rapids/notebooks/slima/outputs/ablation_study_v11"
    ...
    BATCH_SIZE: int = 32  # Increased with gradient accumulation
    LR: float = 2e-4
    ...
    GRAD_ACCUM_STEPS: int = 2  # Gradient accumulation steps for multi-GPU
...
class AdaptiveFaceLoss(nn.Module):
    ...
    def forward(self, features: torch.Tensor, labels: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(batch_size):
            ...
            # cos(θ + m)
            cos_theta_m[i, labels[i]] = target_cos * cos_m - target_sin * sin_m
...
def run_ablation_study():
    ...
    loss_functions = [
        ...
        ('fuzzyarcloss_v3_subcenters', {'s': 30.0, 'm': 0.5, 'tau': 0.5, 'num_subcenters': 3}),
        ('softmax', {'s': 30.0}),
        ...
        # V3 alternatives (expected lower performance)
    ...
    results_path = os.path.join(config.OUT_DIR, 'ablation_results_v11.json')
