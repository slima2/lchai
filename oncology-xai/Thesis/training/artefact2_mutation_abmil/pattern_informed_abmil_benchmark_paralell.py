...
def run_benchmark(cfg: Config):
    ...
    print(f"Found {len(slide_ids, flush=True)} slides with embeddings.")

    # keep only slides that appear in labels
    slide_ids = [s for s in slide_ids if s in labels_df.index]
    print(f"{len(slide_ids, flush=True)} slides have mutation labels.")

    # ── 2. Load all slide data ───────────────────────────────────────────────
    print("Loading slide data …", flush=True)
    all_slides: List[SlideData] = []
    for sid in slide_ids:
        try:
            all_slides.append(SlideData(sid, cfg.data_dir))
        except Exception as e:
            print(f"  WARNING: could not load {sid}: {e}", flush=True)
    slide_ids = [s.slide_id for s in all_slides]
    print(f"Successfully loaded {len(all_slides, flush=True)} slides.")
    ...
    if not has_embeddings:
        print("\n[WARNING] No embeddings.npy found in data/slides/.", flush=True)
        print("  Conditions requiring embeddings will be SKIPPED:", flush=True)
    ...
    for gene in cfg.genes:
        if gene not in labels_df.columns:
            print(f"Gene {gene} not in labels.csv — skipping.", flush=True)
            continue
        ...
        print(f"\n{'='*60}", flush=True)
        print(f"  Gene: {gene}  |  slides: {len(valid_slides, flush=True)}  |  pos: {pos_frac:.1%}")
        print(f"{'='*60}", flush=True)

        if pos_frac < cfg.min_positive_frac or pos_frac > (1 - cfg.min_positive_frac):
            print(f"  Skipping {gene}: class imbalance too extreme ({pos_frac:.1%} positive, flush=True).")
            continue
        ...
        for fold_idx, (tr_idx, val_idx) in enumerate(
            ...
            print(f"\n  ── Fold {fold_idx+1}/{cfg.n_folds} ──", flush=True)
            ...
            # ── Baseline 1: XGBoost ─────────────────────────────────────
            print(f"  [baseline1_xgboost]", flush=True)
            ...
            print(f"    AUROC={metrics['auroc']:.4f}  F1={metrics['f1']:.4f}", flush=True)

            # ── Baseline 2: ABMIL embeddings ─────────────────────────────
            if "baseline2_abmil_embeddings" in active_conditions:
                print(f"  [baseline2_abmil_embeddings]", flush=True)
                ...
                print(f"    AUROC={metrics.get('auroc', float('nan', flush=True)):.4f}  F1={metrics.get('f1', float('nan')):.4f}")

            # ── Baseline 3: ABMIL patterns ────────────────────────────────
            print(f"  [baseline3_abmil_patterns]", flush=True)
            _, metrics = train_abmil(
            ...
            if "proposed_abmil_concat" in active_conditions:
                print(f"  [proposed_abmil_concat]", flush=True)
                _, metrics = train_abmil(
            ...
            if "ablation_abmil_onehot" in active_conditions:
                print(f"  [ablation_abmil_onehot]", flush=True)
                _, metrics = train_abmil(
