...
class SlideData:
    ...
    def __init__(self, slide_id: str, data_dir: str):
        ...
        self.embeddings     = np.load(emb_path).astype(np.float32)    # (N,768)
        ...
        assert self.embeddings.shape[0] == self.pattern_probs.shape[0], \
            f"Tile count mismatch for {slide_id}"
        assert self.embeddings.shape[1] == 768
...
def train_abmil(
    ...
    # infer input dim from feature mode
    dim_map = {
        "embeddings": 768,
        "patterns":   6,
        "concat":     774,
...
def run_benchmark(cfg: Config):
    ...
    # ── 3. Per-gene, per-condition, k-fold benchmark ─────────────────────────
    ...
    summary = build_summary(all_results, cfg)
...
def build_summary(all_results: Dict, cfg: Config) -> pd.DataFrame:
...
def generate_synthetic_data(data_dir: str, n_slides: int = 60, seed: int = 42):
    ...
    for sid in slide_ids:
        ...
        emb     = rng.standard_normal((n_tiles, 768)).astype(np.float32)
