...
# TCGA mutation data (MAF files — supports multiple)
MAF_PATHS = [
    "/home/rapids/notebooks/slima/TGCA MAF/cohortMAF.2025-11-16 only LUAD.maf",
    "/home/rapids/notebooks/slima/TGCA MAF/cohortMAF_LUSC.maf",          # ← add LUSC MAF here
    # Add more MAF paths as needed:
    # "/home/rapids/notebooks/slima/TGCA MAF/data_mutations.txt",        # cBioPortal format
]
# Alternative: pre-processed mutation labels CSV (if exists, skips MAF parsing)
MUTATION_LABELS_PATH = "/home/rapids/notebooks/slima/data/tcga_luad_lusc_mutation_labels.csv"
...
def load_mutation_labels() -> pd.DataFrame:
    """
    Load TCGA mutation labels from multiple MAF files.
    
    Tries in order:
      1. Pre-processed CSV (if exists)
      2. Multiple MAF files (merge all)
      3. Any alternative paths found
    ...
    if os.path.exists(MUTATION_LABELS_PATH):
        ...
        for gene in GENES:
            if gene in df.columns:
                print(f"    {gene:6s}: {df[gene].sum():3d} mutated ({df[gene].sum()/len(df)*100:.1f}%)")
        return df
    
    # Option 2: Multiple MAF files
    all_mutation_dfs = []
    found_mafs = []
    
    for maf_path in MAF_PATHS:
        if os.path.exists(maf_path):
            print(f"  Found MAF: {maf_path}")
            found_mafs.append(maf_path)
        else:
            print(f"  [SKIP] Not found: {maf_path}")
    
    # Also check for common alternative locations
    alt_paths = [
        "/home/rapids/notebooks/slima/TGCA MAF/data_mutations.txt",
        "/home/rapids/notebooks/slima/data/data_mutations.txt",
        "/home/rapids/notebooks/slima/TGCA MAF/lusc_tcga_pan_can_atlas_2018/data_mutations.txt",
    ]
    for p in alt_paths:
        if os.path.exists(p) and p not in found_mafs:
            print(f"  Found alternative MAF: {p}")
            found_mafs.append(p)
    
    if not found_mafs:
        print("  [ERROR] No MAF files found!")
        print("  Please download TCGA-LUSC MAF from GDC Portal:")
        print("    https://portal.gdc.cancer.gov/repository")
        print("  Filter: Project=TCGA-LUSC, Data Type=Masked Somatic Mutation, Format=MAF")
        sys.exit(1)
    
    print(f"\n  Processing {len(found_mafs)} MAF file(s)...")
    
    for maf_path in found_mafs:
        print(f"\n  ── Parsing: {os.path.basename(maf_path)} ──")
        df_maf = parse_maf_file(maf_path)
        if df_maf is not None and len(df_maf) > 0:
            all_mutation_dfs.append(df_maf)
            print(f"     → {len(df_maf)} non-synonymous variants from {df_maf['case_id'].nunique()} cases")
    
    if not all_mutation_dfs:
        print("  [ERROR] No valid mutation data found in any MAF file!")
        sys.exit(1)
    
    # Merge all mutation DataFrames
    df_all = pd.concat(all_mutation_dfs, ignore_index=True)
    print(f"\n  Merged: {len(df_all)} total variants from {df_all['case_id'].nunique()} unique cases")
    
    # Convert to binary labels
    df_labels = create_binary_labels(df_all)
    
    return df_labels


def parse_maf_file(maf_path: str) -> pd.DataFrame:
    """Parse a MAF or cBioPortal mutations file into a standardized DataFrame."""
    import gzip
    
    # Detect format
    is_gzip = maf_path.endswith('.gz')
    is_tsv = maf_path.endswith('.txt') or maf_path.endswith('.tsv')
    ...
    if not rows:
        print(f"     [WARN] No data rows in {maf_path}")
        return None
    
    df = pd.DataFrame(rows)
    
    # Find columns
    gene_col = next((c for c in ['Hugo_Symbol', 'gene', 'Gene', 'hugo_symbol'] if c in df.columns), None)
    case_col = next((c for c in ['Tumor_Sample_Barcode', 'case_id', 'SAMPLE_ID', 'sample_id',
                                  'case_submitter_id', 'Tumor_Sample_UUID'] if c in df.columns), None)
    vc_col = next((c for c in ['Variant_Classification', 'variant_classification', 'Consequence'] if c in df.columns), None)
    
    if gene_col is None or case_col is None:
        print(f"     [WARN] Missing gene/case columns. Available: {list(df.columns[:15])}")
        return None
    
    print(f"     Gene col: {gene_col}, Case col: {case_col}, Variants: {len(df)}")
    ...
    if vc_col:
        ...
        print(f"     Filtered {before - len(df)} synonymous → {len(df)} non-synonymous")
    
    # Extract TCGA case ID (first 3 segments: TCGA-XX-XXXX)
    ...
    return df[['case_id', 'gene']].copy()


def create_binary_labels(df_all: pd.DataFrame) -> pd.DataFrame:
    """Convert merged mutations DataFrame to binary labels per case × gene."""
    
    all_cases = sorted(df_all['case_id'].unique())
    
    # Build gene → set of mutated cases
    gene_cases = {}
    for gene in GENES:
        gene_cases[gene] = set(df_all[df_all['gene'] == gene]['case_id'].unique())
    
    result = []
    for case in all_cases:
        row = {'case_id': case}
        for gene in GENES:
            row[gene] = 1 if case in gene_cases[gene] else 0
        result.append(row)
...
def train_mutation_models(df_profiles: pd.DataFrame, df_labels: pd.DataFrame):
    ...
    if len(df) < len(df_profiles) * 0.5:
        print(f"\n  [DIAGNOSTIC] Low match rate ({len(df)}/{len(df_profiles)} = {len(df)/len(df_profiles)*100:.0f}%)")
        print(f"  Sample profile case_ids: {sorted(df_profiles['case_id'].unique())[:10]}")
        print(f"  Sample label case_ids:   {sorted(df_labels['case_id'].unique())[:10]}")
        
        # Try alternative matching: 4-segment barcode (TCGA-XX-XXXX-XX)
        # Some MAFs use TCGA-XX-XXXX-01A vs slides use TCGA-XX-XXXX-01Z
        # The patient ID is always the first 3 segments
        prof_ids = set(df_profiles['case_id'].unique())
        lab_ids = set(df_labels['case_id'].unique())
        overlap = prof_ids & lab_ids
        print(f"  Direct overlap: {len(overlap)}")
        
        # Try truncating to just TCGA-XX-XXXX (12 chars)
        df_profiles['case_id_short'] = df_profiles['case_id'].str[:12]
        df_labels['case_id_short'] = df_labels['case_id'].str[:12]
        overlap_short = set(df_profiles['case_id_short'].unique()) & set(df_labels['case_id_short'].unique())
        print(f"  Overlap with 12-char truncation: {len(overlap_short)}")
        
        if len(overlap_short) > len(overlap):
            print(f"  → Using 12-char case_id for better matching")
            df_profiles['case_id'] = df_profiles['case_id_short']
            df_labels['case_id'] = df_labels['case_id_short']
            # Re-dedup labels after truncation
            df_labels = df_labels.drop_duplicates(subset='case_id', keep='first')
            df = df_profiles.merge(df_labels, on='case_id', how='inner')
            print(f"  New matched cases: {len(df)}")
...
def main():
    ...
    print("  Multi-MAF support: LUAD + LUSC")
    print("=" * 70)
    
    # Delete cached labels to force re-parse from MAFs
    if os.path.exists(MUTATION_LABELS_PATH):
        os.remove(MUTATION_LABELS_PATH)
        print(f"  [Cleared cached labels: {MUTATION_LABELS_PATH}]")
