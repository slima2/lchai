"""Ontology Admin Service — FastAPI application."""
...
app = FastAPI(title="LCHAI Ontology Admin", description="Ontology management: proposals, validation, publish/rollback", version=settings.version, lifespan=lifespan)
...
async def _exc(request: Request, exc: Exception):
    ...
    return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content=ErrorResponse(errorCode="INTERNAL_ERROR", message="Unexpected error", correlationId=cid).model_dump(by_alias=True))
